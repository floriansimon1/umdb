...
impl ScanResult {
    pub fn to_device(&self, aliases: &BTreeMap<String, String>) -> Device {
        match &self {
...
pub fn adb_devices(configuration: &Configuration) -> Result<Vec<Device>, DeviceListingError> {
    ...
    .split("\n")
    .skip(1)
    .map(|line| parse_line(adb_command, line))
    ...
    Ok(results.iter().map(|result| ScanResult::to_device(result, &aliases)).collect())
}

fn parse_line(adb_command: &str, line: &str) -> Result<ScanResult, DeviceListingError> {
    ...
    if let Some((ip, port)) = try_parse_remote_id(&id) {
        return Ok(ScanResult::IpResult(IpResult { id, is_offline, port, ip }));
    }

    let addresses = find_usb_device_ips(adb_command, &id)?;

    Ok(ScanResult::UsbResult(UsbResult { id, addresses }))
}

fn find_usb_device_ips(adb_command: &str, id: &str) -> Result<Vec<IpAddr>, DeviceListingError> {
